...
برای migration لازم هست پروژه PaymentService.Api به عنوان default project انتخاب گردد و دستور update database اجرا شود. در سایر پروژه ها از دیتابیس استفاده نشده است.
...
در background worker ای که وظیفه expire کردن تراکنش های قدیمی رو داره، بجای استفاده از کوئری های efCore و فچ کردن دیتا در رم و تغییردادن status و آپدیت رکورد ها، از raw sql استفاده شده چون عموما این فرایند ها هزینه بر هستن و صرفا قراره یک سری رکورد رو آپدیت کنن. حتی اگر در یک سیکل به مشکل بخورن، در سیکل بعدی اتوماتیک مشکل حل میشه. بنابراین امکان خطا بسیار پایینه و نمی صرفه حجم عظیمی از دیتا داخل رم لود بشه بخاطر یک تغییر کوچک. بهمین دلیل مستقیما command مد نظر روی sql اجرا میشه که به شدت بازدهی بالاتری داره. 
